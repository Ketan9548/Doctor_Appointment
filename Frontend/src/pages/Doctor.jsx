          ...
          <div className="md:col-span-1 bg-white p-6 rounded-xl shadow-md">
            ...
            <div className="space-y-3">
              {["General physician", "Gynecologist", "Dermatologist", "Pediatricians", "Neurologist", "Gastroenterologist"].map((spec, index) => (
                <p onClick={() => speciality === spec ? navigate('/alldoctor') : navigate(`/alldoctor/${spec}`)} key={index} className="bg-blue-100 px-4 py-2 rounded-md text-center text-gray-600 hover:bg-blue-200 cursor-pointer transition">
                  {spec}
                </p>
              ))}
            </div>
